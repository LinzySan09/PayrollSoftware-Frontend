...
function EmpModal({ title, emp, onClose, onSaved }) {
  ...
  const [form, setForm] = useState(
    emp
      ? { ...emp, basic: emp.basic || "", allowances: emp.allowances || "" }
      : BLANK,
  );
  ...
  const save = async () => {
    if (!form.name || !form.emp_id || !form.dept) {
      toast("Name, ID and Department are required.", "warning");
      return;
    }
    setSaving(true);
    try {
      ...
      toast(
        `${res.data.name} ${emp ? "updated" : "added"} successfully.`,
        "success",
      );
      onSaved(res.data);
      onClose();
...
function ViewModal({ emp, onClose, onEdit }) {
  if (!emp) return null;
  const yrs = emp.joining
    ? (
        (Date.now() - new Date(emp.joining)) /
        (1000 * 60 * 60 * 24 * 365.25)
      ).toFixed(2)
    : "—";
  return (
    <div className="modal-bg bg-black" onClick={onClose}>
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        <div className="modal-hd">
        ...
        >
          ...
          <div style={{ flex: 1 }}>
            ...
            <div
              style={{ fontSize: ".8rem", color: "var(--txt3)", marginTop: 3 }}
            >
              {emp.designation} · {emp.dept}
            </div>
            <div
              style={{ fontSize: ".75rem", color: "var(--txt3)", marginTop: 2 }}
            >
              {emp.emp_id}
            </div>
          </div>
          <span
            className={`badge ${emp.status === "Active" ? "b-green" : "b-gray"}`}
          >
            {emp.status}
          </span>
        </div>
        <div className="kv-grid">
          {[
            ...
            <div className="kv-item" key={k}>
              <div className="kv-lbl">{k}</div>
              <div className="kv-val">{v || "—"}</div>
            </div>
          ))}
        </div>
        <label style={{ marginBottom: ".65rem", display: "block" }}>
          Document Expiry
        </label>
        <div className="doc-grid">
          {[
            ["Passport", emp.passport_exp],
            ["Emirates ID", emp.emirates_exp],
            ["Visa", emp.visa_exp],
          ].map(([doc, exp]) => (
  ...
  const save = async () => {
    ...
    try {
      const res = await updateEmployee(emp.id, {
        ...draft,
        basic: parseFloat(draft.basic) || 0,
        allowances: parseFloat(draft.allowances) || 0,
      });
    ...
    <tr className={editing ? "row-editing" : ""}>
      ...
      <td>
        {editing ? (
          ...
          <span style={{ color: "var(--txt2)", fontSize: ".82rem" }}>
            {emp.designation}
          </span>
        )}
      </td>
      <td>
        {editing ? (
          ...
          <span style={{ fontWeight: 600 }}>
            AED {(emp.basic || 0).toLocaleString()}
          </span>
        )}
      </td>
      <td>
        <span className="badge b-blue">{yrs} yrs</span>
      </td>
      <td>
        <ExpiryBadge dateStr={emp.visa_exp} />
      </td>
      <td>
        {editing ? (
          <select
